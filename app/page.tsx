...
const ProjectCard = ({ name, description, language, languageColor, stars, forks }: any) => (
  <div className="bg-white border border-gray-200 rounded-lg p-5 hover:shadow-md transition-shadow cursor-default flex flex-col h-full">
    <div className="flex items-start justify-between mb-2">
      <div className="flex items-center space-x-2">
        <svg className="w-4 h-4 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 7v10a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-6l-2-2H5a2 2 0 00-2 2z" />
        </svg>
        <a href="#" className="text-[15px] font-semibold text-blue-600 hover:underline">{name}</a>
      </div>
      <span className="text-[11px] font-medium text-gray-500 border border-gray-200 rounded-full px-2 py-0.5">Public</span>
    </div>
    <p className="text-[13px] text-gray-600 mb-4 flex-grow leading-relaxed">{description}</p>
    <div className="flex items-center text-[12px] text-gray-500 space-x-4 mt-auto">
      {language && (
        <div className="flex items-center">
          <span className={`w-3 h-3 rounded-full mr-1.5 ${languageColor}`}></span>
          {language}
        </div>
      )}
      {stars !== undefined && (
...
const ProjectsWindowContent = ({ searchQuery }: { searchQuery: string }) => {
  const projects = [
    ...
    },
    {
      ...
      description: 'a minimal code editor for agents',
      language: 'TypeScript',
      languageColor: 'bg-blue-600',
    },
    {
      name: 'Logicarium',
      description: 'Logicarium is a minimalist, performant, visual logic design environment. Design complex digital systems using a high-density, performant interface. Custom gates for modular logic, and a visual DSL ...',
      language: 'C++',
  ...
  return (
    <div className="p-6 bg-[#FAFAFA]/80 rounded-b-xl h-[500px] overflow-y-auto">
